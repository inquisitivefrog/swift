//
//  MatrixMaterialsGameView.swift
//  DinoGames
//
//  Matrix Materials: Help the paleontologist identify the matrix material encasing the fossil.
...
struct MatrixMaterial: Identifiable {
    let id: Int
    let name: String
    /// Slug for asset names (e.g. "limestone", "sandstone").
    var materialSlug: String { name.lowercased().replacingOccurrences(of: " ", with: "-") }
    /// Image set name: matrix-{stone}. Fallback when no matrix-{stone}-{dinosaur} asset.
    var imageName: String? { "matrix-\(materialSlug)" }
    /// Image set name when combined with a dinosaur (Dino Lunch style): matrix-{stone}-{dinosaur}.
    func imageName(dinosaurSlug: String?) -> String? {
        guard let slug = dinosaurSlug, !slug.isEmpty else { return imageName }
        return "matrix-\(materialSlug)-\(slug)"
    }
}

struct MatrixMaterialsRound: Identifiable {
    let id: Int
...
struct MatrixMaterialsGameConfig {
    let id: String
    let title: String
    let introAudio: String
    let rounds: [MatrixMaterialsRound]
    let allMaterials: [MatrixMaterial]
}

// MARK: - Main View

struct MatrixMaterialsGameView: View {
    @Binding var isPresented: Bool
    let gameConfig: MatrixMaterialsGameConfig
    ...
    private var currentQuestion: MatrixMaterialsRound? {
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 24) {
                ...
                if let question = currentQuestion, !isGameComplete {
                    VStack(spacing: 28) {
                        ...
                        // Bottom: 3 material options (walk then tap). Use matrix-{stone} only so the choices are stones, not dinosaurs.
                        HStack(spacing: 10) {
            ...
            .onAppear {
                ...
                // Options walk starts after game-matrix-identify-the-stone finishes (see round prompt onAppear).
            }
            .onChange(of: currentRound) { _, newRound in
                guard newRound >= 2, newRound <= 3, currentQuestion != nil else { return }
                isAudioPlaying = true
                speechManager.speak("game-matrix-identify-the-stone")
    ...
    private func roundPromptView(question: MatrixMaterialsRound) -> some View {
        Group {
            if let dino = question.dinosaur {
                ...
                let compositeName = correctMaterial.flatMap { mat in
                    dinosaurAssetSlug(dino).map { "matrix-\(mat.materialSlug)-\($0)" }
                }
                VStack(spacing: 8) {
    ...
    private func handleMaterialTap(_ material: MatrixMaterial, question: MatrixMaterialsRound) {
    ...
    private func checkAnswer(material: MatrixMaterial, question: MatrixMaterialsRound) {
...
struct MatrixMaterialOptionCard: View {
    ...
    /// Use matrix-{stone} only so the three choices show stones, not dinosaur composites.
    private var resolvedImageName: String? {
        guard let name = material.imageName, UIImage(named: name) != nil else { return nil }
        return name
    }
...
/// picks one correct material per round. Image sets matrix-{stone}-{dinosaur} may exist for
...
struct MatrixMaterialsGameConfigs {
    ...
    /// Materials that have an existing matrix-{stone}-{dinosaur} image set for this dinosaur. Stone is derived from the image set label.
    private static func materialsWithImageSet(for dino: Dinosaur) -> [MatrixMaterial] {
        ...
        return allMaterials.filter { mat in
            UIImage(named: "matrix-\(mat.materialSlug)-\(slug)") != nil
        }
    }

    /// All (dinosaur, material) pairs that have a matrix-{stone}-{dinosaur} image set. Stone is identified from the image set label.
    ...
    static var matrixMaterials: MatrixMaterialsGameConfig {
        let pool = allMaterials
        let candidates = roundCandidates.shuffled()
        ...
        guard selected.count == 3 else {
            fatalError("Matrix Materials: need at least 3 distinct stones with matrix-{stone}-{dinosaur} image sets (found \(selected.count) distinct)")
        }

        var rounds: [MatrixMaterialsRound] = []
        var usedMaterialIdsAcrossRounds: Set<Int> = []
        for (idx, (dino, correct)) in selected.enumerated() {
            // One option must match the dinosaur image (matrix-{stone}-{dinosaur}); the other two are decoys.
            ...
            rounds.append(MatrixMaterialsRound(
                id: idx + 1,
                dinosaur: dino,
        ...
        return MatrixMaterialsGameConfig(
            id: "matrix-materials",
            title: "Matrix Materials!",
            introAudio: "game-matrix-materials",
            rounds: rounds,
            allMaterials: pool
        )
    }
...
#Preview {
    MatrixMaterialsGameView(isPresented: .constant(true), gameConfig: MatrixMaterialsGameConfigs.matrixMaterials)
}
