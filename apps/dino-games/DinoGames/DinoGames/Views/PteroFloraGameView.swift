...
struct PteroFloraGameView: View {
    ...
    private var gameBody: some View {
        if let p = plant, !isGameComplete {
            GeometryReader { geometry in
                ...
                // Match Dino Flora iPad layout: larger plant, selectable circles, and chrome fonts.
                let playMaxScale: CGFloat = 1.85
                ...
                let circleSize = GameCatalogImageMetrics.scaled(pteroFloraCircleSize, safeWidth: safeWidth, maxScale: playMaxScale)
    ...
    private func plantImage(_ p: PteroFloraPlant, maxWidth: CGFloat, maxHeight: CGFloat) -> some View {
    ...
    private func fiveStarLayout(height: CGFloat, circleSize: CGFloat, radius: CGFloat) -> some View {
        PteroFloraStarLayoutView(
            slots: slots,
            matchedIds: matchedIds,
            introHighlightIndex: introWalkIndex,
            circleSize: circleSize,
            radius: radius,
            tapHandler: PteroFloraTapHandler(perform: handleTap)
        )
            .frame(height: height)
            .padding(.horizontal)
    }
            ...
            ZStack(alignment: .center) {
                ForEach(Array(slots.enumerated()), id: \.offset) { index, creature in
                    PteroFloraCircleView(
                        ...
                        .position(
                            x: geo.size.width / 2 + fittedRadius * CGFloat(cos(pteroFormationsStarAngles[index])),
                            y: geo.size.height / 2 + 20 + fittedRadius * CGFloat(sin(pteroFormationsStarAngles[index]))
                        )
...
private struct PteroFloraCircleView: View {
    ...
    var size: CGFloat = pteroFloraCircleSize
