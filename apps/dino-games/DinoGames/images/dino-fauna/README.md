# dino-fauna (unreleased)

This game is **not** in shipping levels 1–4.

- **Xcode imagesets** (`Assets.xcassets/Dinosaur-Fauna`) live on git branch **`future-games`** (see also `origin/future-games`). They are **not** on `main`, so they are **not** in the IPA.
...
git checkout future-games -- \
  apps/dino-games/DinoGames/images/dino-fauna \
  apps/dino-games/DinoGames/DinoGames/Assets.xcassets/Dinosaur-Fauna
git lfs pull
```
